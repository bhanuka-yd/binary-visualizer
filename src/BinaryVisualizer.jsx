...
export default function BinaryVisualizer() {
  ...
  const activeBits = bits
    .map((bit, index) => ({
      position: 31 - index,
      value: bit ? Math.pow(2, 31 - index) : 0,
    }))
    .filter((bit) => bit.value > 0)
    .sort((a, b) => b.position - a.position);

  return (
    <div className="bg-gray-900 min-h-screen text-gray-100 p-4 flex flex-row">
      <div className="w-auto mx-auto">
        <h1 className="text-2xl font-bold mb-2 text-center">
          Binary to Decimal Converter
        </h1>
        <h6 className="text-center mb-8 text-gray-500">
          (click on bits to set them)
        </h6>

        <div className="text-center mb-8">
          <div className="text-5xl font-mono mb-2">
            {getDecimalValue().toLocaleString()}
          </div>
          <div className="text-gray-400">Decimal Value</div>
        </div>

        <div className="bg-gray-800 p-4 rounded-lg mb-8 py-12">
